      ...
      <form className="project-form" onSubmit={(event) => void submit(event)} noValidate>
        ...
        <section className="form-section" aria-labelledby="intent-heading">
          ...
          <div className="form-grid">
            <label className="full-field"><span>Project name *</span><input required {...form.register("name")} />{form.formState.errors.name ? <small className="field-error">{form.formState.errors.name.message}</small> : null}</label>
            <label className="full-field"><span>Goal *</span><textarea rows={4} {...form.register("goal")} />{form.formState.errors.goal ? <small className="field-error">{form.formState.errors.goal.message}</small> : null}</label>
        ...
        <section className="form-section" aria-labelledby="delivery-heading">
          ...
          <div className="form-grid">
            ...
            <label><span>Deadline</span><input type="date" {...form.register("deadline")} />{form.formState.errors.deadline ? <small className="field-error">{form.formState.errors.deadline.message}</small> : null}</label>
            ...
            <label className="full-field"><span>IANA timezone</span><input {...form.register("timezone")} /></label>
          </div>
        </section>
        <section className="form-section" aria-labelledby="scope-heading">
          <div className="form-section-heading"><span>03</span><div><h2 id="scope-heading">Scope signals</h2><p>Enter one feature per line.</p></div></div>
          <div className="form-grid">
        ...
        <div className="form-actions">
          ...
          <button className="button primary" type="submit" disabled={mutation.isPending}>{mutation.isPending ? "Creating project…" : "Create project"}</button>
        </div>
      </form>
    </div>
  );
